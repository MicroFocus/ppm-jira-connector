...
public class JIRAService {
    ...
    public List<JIRAIssue> getIssues(String projectKey, Map<String, Boolean> map) {
        ...
        ClientResponse response =
                wrapper.sendGet(baseUri + JIRAConstants.ISSUES_SUFFIX + projectKey + getIssueQueryString(map));
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                if (!obj.getBoolean("subtask")) {
                    JIRAIssueType jit =
                            new JIRAIssueType(obj.getString("self"), obj.getString("id"), obj.getString("description"),
                                    obj.getString("iconUrl"), obj.getString("name"), obj.getBoolean("subtask"),
                                    obj.getInt("avatarId"));
            ...
            XMLGregorianCalendar dateTo, String projectKey)
    {
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                JIRATempoIssue jti =
                        new JIRATempoIssue(isuSelf, isuId, isuProId, isuKey, remainingEstimateSeconds, isuType,
                                isuSummary);
            ...
            XMLGregorianCalendar dateTo, String projectKey)
    {
        ...
        String requestParameter =
                "worklogDate>=" + dateFrom.toString().substring(0, 10) + " and worklogDate<=" + dateTo.toString()
                        .substring(0, 10);

        requestParameter += "".equals(projectKey) ? "" : " and project=" + projectKey;

        ClientResponse response =
                wrapper.sendGet(baseUri + JIRAConstants.ISSUES_WORKLOGS_SUFFIX + encodeUrl(requestParameter));
            ...
            String actualFinish, String sprintCustomId) throws JSONException
    {
        ...
        if (!isSubtask) {
            ...
            for (int i = 0; i < childrenJson.length(); i++) {
                ...
                JIRAIssue subIssue =
                        resolveIssue(subfields, true, scheduledStart, scheduledFinish, actualFinish, sprintCustomId);
